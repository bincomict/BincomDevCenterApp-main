...
export const subscribeToAllState = (
  ...
  const unsubscribes = collectionsToListen.map(colName => {
    ...
    if (colName === "profiles") {
      ...
      if (isAdmin) {
        ...
        // Approach A: Date-scoping via string comparison (YYYY-MM-DD format on 'date' field)
        const daysAgoStr = getLagosDateStringDaysAgo(45);
        queryRef = query(collection(db, "attendance"), where("date", ">=", daysAgoStr));

        // Approach B: Date-scoping via Firestore Timestamp comparison (if 'createdAt' or 'timestamp' exists)
        // const daysAgoTimestamp = Timestamp.fromDate(new Date(Date.now() - 45 * 24 * 60 * 60 * 1000));
        // queryRef = query(collection(db, "attendance"), where("createdAt", ">=", daysAgoTimestamp));
      ...
      if (isAdmin) {
        ...
        // Approach A: Date-scoping via string comparison (YYYY-MM-DD format on 'date' field)
        const daysAgoStr = getLagosDateStringDaysAgo(45);
        queryRef = query(collection(db, "meetingHistory"), where("date", ">=", daysAgoStr));

        // Approach B: Date-scoping via Firestore Timestamp comparison (if 'createdAt' or 'timestamp' exists)
        // const daysAgoTimestamp = Timestamp.fromDate(new Date(Date.now() - 45 * 24 * 60 * 60 * 1000));
        // queryRef = query(collection(db, "meetingHistory"), where("createdAt", ">=", daysAgoTimestamp));
  ...
  const checkInterval = setInterval(() => {
    ...
    // Use the distributed lock transaction to run midnight sync & auto-archive if needed
  ...
  }, 900000); // 15 minutes instead of 5 minutes to conserve reads/writes
...
// Defaulting to 15 minutes (900000ms) to conserve reads/writes and prevent Spark quota exhaustion.
// Tradeoff: Auto-archive checks and automated midnight sync run less frequently, but the firestore read/write volume is dramatically reduced.
export const STATE_SYNC_POLLING_INTERVAL = 900000;
...
export const runMidnightSyncIfNeeded = async (
  ...
  // 1. Quick local check first to avoid transaction overhead
  // Check if timezone is Lagos midnight (00:00 or 00:01)
  let isMidnightWindow = false;
  try {
    ...
    if (currentLagosTime === "00:00" || currentLagosTime === "00:01") {
      isMidnightWindow = true;
    }
  } catch (e) {
    console.error("Error formatting Lagos time in runMidnightSyncIfNeeded:", e);
  }

  if (!isMidnightWindow && !force) {
    return false;
  }
  ...
  try {
    const result = await runTransaction(db, async (transaction) => {
      ...
      // Check if lastRunDate is already today -> skip sync
      if (lastRunDate === todayStr && !force) {
        return { claimSucceeded: false, reason: "Already run today" };
      }
      ...
      if (lockedAt) {
        ...
        if (lockAgeMs < 300000 && lockedBy !== localSessionId) {
    ...
    if (!result.claimSucceeded) {
      console.log(`[midnightSync] Did not acquire lock. Reason: ${result.reason}`);
      return false;
    }

    console.log(`[midnightSync] Lock acquired successfully by session: ${localSessionId}. Executing sync & auto-archive...`);

    // Proceed to run the sync
