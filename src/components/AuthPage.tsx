...
import { loginUser, registerUser, adminBypassLogin, sendPasswordRecoveryEmail } from "../firebaseService";
...
export default function AuthPage({ onAuthSuccess, profiles }: AuthPageProps) {
  ...
  const [recoveryModalError, setRecoveryModalError] = useState("");
  ...
  const handleRecovery = async (e: React.FormEvent) => {
    ...
    try {
      const sentToEmail = await sendPasswordRecoveryEmail(recoveryEmail);
      setRecoverySent(false);
      setShowRecoveryModal(false);
      setRecoveryEmail("");
      setSuccess(`A password reset link has been sent to ${sentToEmail}. Please check your inbox and spam folder.`);
    } catch (err: any) {
      setRecoverySent(false);
      setRecoveryModalError(err.message || "Failed to send password recovery link. Please try again.");
    }
  };
          ...
          <form onSubmit={handleLogin} className="space-y-4" id="login-form">
            ...
            <div className="pt-2 flex items-center justify-between text-xs text-gray-400 font-medium select-none">
              <button
                id="recover-pwd-btn"
                type="button"
                onClick={() => { setShowRecoveryModal(true); setRecoverySent(false); setRecoveryModalError(""); }}
      ...
      {showRecoveryModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in" id="recovery-modal-overlay">
          <div className="bg-white rounded-2xl max-w-[390px] w-full border border-gray-100 shadow-xl p-6 relative" id="recovery-modal-body">
            ...
            {recoveryModalError && (
              <div className="p-3 bg-rose-50 text-rose-800 text-xs rounded-lg mb-4 border border-rose-100/50 leading-relaxed font-medium" id="recovery-modal-error">
                {recoveryModalError}
              </div>
            )}
