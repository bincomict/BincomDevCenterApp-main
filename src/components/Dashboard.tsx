...
export default function Dashboard({ 
  ...
  const getAttendanceForMeeting = (meetingId: string) => {
    const matches = (attendance || []).filter((a: any) => a.userId === profile.id && a.meetingId === meetingId);
  ...
  const renderMeetingCard = (p: any) => {
    ...
    const userAtts = (attendance || []).filter((a: any) => a.userId === profile.id && a.meetingId === p.id);
    const record = userAtts.find((a: any) => a.status === "Attended" || a.status === "Late") || userAtts[0];
    let attendanceText = "";
    let attendanceColor = "";
    if (record) {
