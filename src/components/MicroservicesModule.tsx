...
export default function MicroservicesModule({ 
  ...
  const handleJoinKD = async () => {
    ...
    try {
      ...
      if (onJoinMeeting) {
        const kdMeet = (state.meetings || []).find((m: any) =>
          m.type === "knowledge" || (m.title || "").toLowerCase().includes("kd") || (m.title || "").toLowerCase().includes("knowledge")
        );
        onJoinMeeting(kdMeet ? kdMeet.id : "meet_2");
      }
          ...
          <form onSubmit={handleStandupSubmit} className="md:col-span-5 bg-white rounded-2xl border border-gray-150 p-5 space-y-4" id="standup-log-form">
            ...
            <div className="bg-emerald-50 border border-emerald-150 rounded-xl p-3.5 space-y-2">
              ...
              <a
                ...
                onClick={() => {
                  if (onJoinMeeting) {
                    const matchMeet = (state.meetings || []).find((m: any) =>
                      (m.title || "").toLowerCase().includes("stand") ||
                      (m.title || "").toLowerCase().includes("alignment")
                    );
                    onJoinMeeting(matchMeet ? matchMeet.id : "meet_1");
                  }
                }}
