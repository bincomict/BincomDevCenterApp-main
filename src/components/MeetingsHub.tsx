...
export default function MeetingsHub({ 
  ...
  const getAttendanceForMeeting = (meetingId: string) => {
    const matches = attendance.filter((a) => a.userId === profile.id && a.meetingId === meetingId);
                      ...
                      <div className="flex items-center justify-between w-full">
                        {index ? (
                          <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded text-[10px] font-extrabold ${
                            ...
                            {index.status === "Missed" ? "Missed" : index.status === "Late" || (index.status || "").toLowerCase().includes("late") ? "Late Check-In" : "Attended On Time"}
                          </span>
                        ) : (
