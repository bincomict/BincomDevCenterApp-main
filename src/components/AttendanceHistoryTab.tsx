...
import { adminUpdateAttendance, subscribeToAuditLogs, parseMeetingTimeToMinutes, formatMinutesToMeetingTime, saveAttendancePunctualityConfig, getLagosMinutesPastMidnight, reclassifyAllAttendanceRecords, isUserEligibleForMeetingInBackend } from "../firebaseService";
...
export default function AttendanceHistoryTab({
  ...
  const isUserEligibleForMeeting = (user: Profile, meeting: MeetingHistoryRecord): boolean => {
    return isUserEligibleForMeetingInBackend(user, meeting, state.meetingAssignments || []);
  };
