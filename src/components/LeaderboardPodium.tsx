...
import { Profile, AttendanceRecord, KDPresentation, Meeting, KnowledgeDevelopmentInfo } from "../types";
import { Trophy, Medal, Star, ShieldCheck, Award, BookOpen, Layers } from "lucide-react";
...
export default function LeaderboardPodium({ 
  ...
  const [boardType, setBoardType] = useState<"general" | "kd">("kd");
  ...
  return (
    <div className="space-y-6" id="leaderboard-root">
      ...
      {/* MAIN TOP SUB-TAB SELECTOR */}
      <div className="flex flex-wrap items-center gap-2 p-2 bg-gray-100 rounded-2xl border border-gray-200">
        <button
          onClick={() => setBoardType("kd")}
          className={`flex-1 min-w-[200px] py-2.5 px-4 text-xs font-black rounded-xl transition flex items-center justify-center gap-2 cursor-pointer ${
            boardType === "kd"
              ? "bg-[#4B5E40] text-white shadow-sm"
              : "text-gray-700 hover:bg-gray-200/80"
          }`}
        >
          <BookOpen className="w-4 h-4 text-emerald-300" /> Knowledge Development (KD) Leaderboard
        </button>
        ...
        >
          <Layers className="w-4 h-4 text-amber-300" /> General Platform Accountability
        </button>
      </div>

      {boardType === "kd" && profile ? (
        <KDLeaderboard
          profile={profile}
          profiles={profiles}
